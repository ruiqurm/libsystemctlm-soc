...
public:
	...
	sc_signal<sc_bv<64>> xdmaChannel_h2cDescByp_src_addr;
	sc_signal<uint32_t> xdmaChannel_h2cDescByp_len;
	sc_signal<sc_bv<64>> xdmaChannel_h2cDescByp_dst_addr;
	...
	sc_signal<sc_bv<64>> xdmaChannel_c2hDescByp_src_addr;
	sc_signal<uint32_t> xdmaChannel_c2hDescByp_len;
	sc_signal<sc_bv<64>> xdmaChannel_c2hDescByp_dst_addr;
	...
	template <typename T> void connect_xdma(T *dev) {
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_h2c.load(xdmaChannel_h2cDescByp_load);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_h2c.src_addr(xdmaChannel_h2cDescByp_src_addr);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_h2c.len(xdmaChannel_h2cDescByp_len);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_h2c.dst_addr(xdmaChannel_h2cDescByp_dst_addr);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_h2c.ctl(xdmaChannel_h2cDescByp_ctl);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_h2c.ready(xdmaChannel_h2cDescByp_ready);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_c2h.load(xdmaChannel_c2hDescByp_load);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_c2h.src_addr(xdmaChannel_c2hDescByp_src_addr);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_c2h.len(xdmaChannel_c2hDescByp_len);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_c2h.dst_addr(xdmaChannel_c2hDescByp_dst_addr);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_c2h.ctl(xdmaChannel_c2hDescByp_ctl);
		dev->descriptor_bypass_channels[0].dsc_bypass_bridge_c2h.ready(xdmaChannel_c2hDescByp_ready);
		dev->descriptor_bypass_channels[0].h2c_bridge.tlast(xdmaChannel_rawH2cAxiStream_tlast);
		dev->descriptor_bypass_channels[0].h2c_bridge.tuser(xdmaChannel_rawH2cAxiStream_tuser);
		dev->descriptor_bypass_channels[0].h2c_bridge.tstrb(xdmaChannel_rawH2cAxiStream_tkeep);
		dev->descriptor_bypass_channels[0].h2c_bridge.tready(xdmaChannel_rawH2cAxiStream_tready);
		dev->descriptor_bypass_channels[0].h2c_bridge.tvalid(xdmaChannel_rawH2cAxiStream_tvalid);
		dev->descriptor_bypass_channels[0].h2c_bridge.tdata(xdmaChannel_rawH2cAxiStream_tdata);
		dev->descriptor_bypass_channels[0].c2h_bridge.tlast(xdmaChannel_rawC2hAxiStream_tlast);
		dev->descriptor_bypass_channels[0].c2h_bridge.tuser(xdmaChannel_rawC2hAxiStream_tuser);
		dev->descriptor_bypass_channels[0].c2h_bridge.tstrb(xdmaChannel_rawC2hAxiStream_tkeep);
		dev->descriptor_bypass_channels[0].c2h_bridge.tready(xdmaChannel_rawC2hAxiStream_tready);
		dev->descriptor_bypass_channels[0].c2h_bridge.tvalid(xdmaChannel_rawC2hAxiStream_tvalid);
		dev->descriptor_bypass_channels[0].c2h_bridge.tdata(xdmaChannel_rawC2hAxiStream_tdata);
	}
	template <typename T> void connect_user_logic(T &dev) {connect_user_logic(&dev);}
	template <typename T> void connect_xdma(T &dev) { connect_xdma(&dev); }
	xdma_bypass_signal(sc_core::sc_module_name name):sc_module(name),
		...
		xdmaChannel_rawH2cAxiStream_tready("xdmaChannel_rawH2cAxiStream_tready"),
		xdmaChannel_rawH2cAxiStream_tvalid("xdmaChannel_rawH2cAxiStream_tvalid"),
		...
		xdmaChannel_rawC2hAxiStream_tready("xdmaChannel_rawC2hAxiStream_tready"),
		xdmaChannel_rawC2hAxiStream_tvalid("xdmaChannel_rawC2hAxiStream_tvalid"),
...
public:
  ...
  type_adapter<sc_bv<3>,uint32_t,bvn_to_uint32t<3>> axilRegBlock_awprot_typeAdpater;
  sc_signal<bool> axilRegBlock_awready;
  sc_signal<bool> axilRegBlock_wvalid;
  type_adapter<sc_bv<4>,uint32_t,bvn_to_uint32t<4>> axilRegBlock_wstrb_typeAdpater;
  sc_signal<bool> axilRegBlock_wready;
  sc_signal<bool> axilRegBlock_bvalid;
  type_adapter<uint32_t,sc_bv<2>,uint32t_to_bvn<2>> axilRegBlock_bresp_typeAdpater;
  sc_signal<bool> axilRegBlock_bready;
  sc_signal<bool> axilRegBlock_arvalid;
  type_adapter<sc_bv<3>,uint32_t,bvn_to_uint32t<3>> axilRegBlock_arprot_typeAdpater;
  sc_signal<bool> axilRegBlock_arready;
  sc_signal<bool> axilRegBlock_rvalid;
  type_adapter<uint32_t,sc_bv<2>,uint32t_to_bvn<2>> axilRegBlock_rresp_typeAdpater;
  sc_signal<bool> axilRegBlock_rready;
  type_adapter<sc_bv<32>,uint32_t,bvn_to_uint32t<32>> axilRegBlock_awaddr_typeAdpater;
  type_adapter<sc_bv<32>,uint32_t,bvn_to_uint32t<32>> axilRegBlock_araddr_typeAdpater;
  sc_signal<sc_bv<32>> axilRegBlock_wdata;
  sc_signal<sc_bv<32>> axilRegBlock_rdata;
  template <typename T> void connect_xdma(T &dev) {
    connect_xdma(&dev);
  }
  template <typename T> void connect_xdma(T *dev) {
  ...
  xdma_signal(sc_core::sc_module_name name)
      : sc_module(name), bypass_channel("xdma-bypass-signal"),
